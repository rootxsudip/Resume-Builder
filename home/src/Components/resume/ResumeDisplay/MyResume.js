...
import dataStoreSlice from '../../../ReduxManager/dataStoreSlice'
...
function MyResume() {
    ...
    const saveResume = async () => {
        const personalInfo = dataStore.personalInfo
        const workEx = dataStore.workEx
        const education = dataStore.education
        const sklls = dataStore.skills
        const imageFile = dataStore.imageFile
        const dbResume = {
            personalInfo
        }
    ...
    try {
        const temp = dataStore.selectedTemplate
        const response = await axios.post(
          `http://localhost:8080/api/users/resumes/templates/${temp.split(" ")[1]}`,
          { userInfo: {...personalInfo}, workExperience: workEx, edu: education, skills: sklls, profile_pic: imageFile },
          {
            headers: {
    ...
    const updateResume = async () => {
        ...
        const resumeId = dataStore.id;
        // console.log(resumeId.toString())
    try {
        const response = await axios.put(
          `http://localhost:8080/api/users/resumes/${resumeId}`,
          { userInfo: {...personalInfo}, workExperience: workEx, edu: education, skills: sklls, profile_pic: imageFile },
          {
            headers: {
              Authorization: `Bearer ${auth.accessToken}`,
    ...
    return (
        ...
        <div  className=' row mt-2 p-5'>
            <div className='w-100 d-flex justify-content-center'>
                ...
                <button className='btn btn-success ms-3 p-2'onClick={saveResume}>
                    Save Resume
                </button>
                <button className='btn btn-success ms-3 p-2'onClick={updateResume}>
                    Update Resume
                </button>
            </div>
        </div>
